...
- `리뷰 요청` 단계에서는 `07_SUMMARY.md`가 MR 메타의 single source of truth여야 하며 `## Merge Request` 섹션을 포함해야 한다.
...
- `리뷰 요청` 단계는 `07_SUMMARY.md` 기반 작업 노트 갱신, canonical helper 기반 MR 생성 또는 재사용, `08_LOOP.md` 전문의 Jira description `### 루프 히스토리` 섹션 동기화까지 끝나야 완료다.
...
- 티켓 loop가 terminal state에 도달하면 메인 세션은 티켓 단위 `git commit`과 `git push`를 수행해야 한다.
- `needs_retry`는 terminal 상태가 아니므로 그 자체로는 티켓 단위 commit/push 대상이 아니다.
