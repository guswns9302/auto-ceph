...
- intake 완료의 최소 실행 입력은 Jira description의 `repo`, `remote`, `endpoint`다.
...
- 고정 순서는 `문제 확인 -> 문제 검토 -> 계획 -> 수행 -> 검증 -> 리뷰 요청`이다.
...
- API 티켓이면 `03_PLAN.md`에 검증용 API 호출 스펙이 있어야 한다.
- `05_UAT.md`, `06_SUMMARY.md` 없이 리뷰 요청으로 넘어가면 안 된다.
- Ralph loop 상태는 `07_LOOP.md`에 누적한다.
...
- API 검증은 `doc/VERIFY_ENV.md`와 계획/수행 단계의 호출 스펙을 사용한 실제 HTTP 호출 결과를 포함해야 한다.
- Jira 입력 누락, repo mismatch, verify 환경설정 누락, branch preparation 불일치 같은 입력/설정 오류는 Ralph loop 자동 재진입 대상이 아니다.
