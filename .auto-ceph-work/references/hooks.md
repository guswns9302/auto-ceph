...
기본 경로에서는 `npx @eddy_yun/auto-ceph-work install --project <path>`가 프로젝트 로컬 `.codex/config.toml`과 `.codex/hooks.json`을 자동으로 패치한다.

설치기는 아래를 보장한다.

- `[features] codex_hooks = true`
- `.codex/config.toml`에는 inline `[[hooks]]` 또는 `[[hooks.hooks]]` 블록을 쓰지 않음
- `.codex/hooks.json`의 `PreToolUse`에 `aceph-prompt-guard.js`와 `aceph-workflow-guard.js` command hook 등록
- hook command가 대상 프로젝트의 `.codex/hooks/*.js` 절대경로를 가리키도록 설정

재설치 또는 업데이트 시 `.codex/hooks.json`의 Auto-Ceph hook은 중복 생성되지 않고 교체된다. 제거 시에는 설치기가 추가한 hook entry만 삭제하고 사용자 hook은 보존한다.
