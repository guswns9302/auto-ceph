...
## E2E Follow-Up Ticket Rules

- E2E 실패 후속 티켓은 사용자 입력 없이 Jira `Task`로 생성한다.
- `$auto-ceph-approval` 후속 제목은 `[ACW] <원본 티켓> E2E 실패 후속 조치` 형식이다.
- `$auto-ceph-e2e` 후속 제목은 `[ACW] <E2E 티켓 ID> E2E 실패 후속 조치 - <기능명>` 형식이다.
- description은 `.auto-ceph-work/references/jira-ticket-template.md`의 intake 가능한 구조를 따른다.
- `### 프로젝트`에는 `repo`와 `remote`를 반드시 포함하고 `remote`는 항상 `origin`으로 고정한다.
- `### 문제점`에는 원본/E2E 티켓 링크, 선택 메뉴 또는 E2E 시나리오, 실패 step, 실제 오류/증거, 기대 결과 대비 차이를 요약한다.
- `### 개선 방향`에는 실패 원인에 대한 수정 방향을 agent가 판단해 작성한다.
- `### 작업 노트` 초기값은 `#### 문제 확인`과 `- 시작 전`이다.
- 생성 직후 active sprint 배정 규칙과 `TO DO` 전이 규칙을 동일하게 적용한다.
- 후속 티켓은 생성만 하고 같은 skill 실행 안에서 즉시 처리하지 않는다.

## E2E Failure Repo Mapping

- `remote-ceph-admin`: UI 렌더링, 화면 이동, 버튼/폼 동작, 프론트 validation, 접근성/DOM selector 문제.
- `ceph-service-api`: REST/API 응답, 비즈니스 로직, DB/CQRS command/query 처리, 서버 validation 문제.
- `ceph-api-gateway`: 로그인 이후 401/403, RBAC 권한, 인증/인가 필터, 라우팅/토큰 전달 문제.
- `ceph-service-scheduler`: Ceph 리소스 수집 지연/누락, quartz job, CQRS read model 갱신 지연, 배치성 데이터 동기화 문제.
- 원인이 복합적이면 가장 직접적인 실패 원인을 우선하고 판단 근거를 후속 티켓 `### 문제점`에 남긴다.
