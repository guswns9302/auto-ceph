...
- 시나리오 최초 작성 시 각 numbered step 아래에 `  - 기대결과: ...`를 함께 작성한다.
- E2E 실행 후 parent skill은 각 numbered step 아래에 결과 annotation만 추가 또는 갱신할 수 있다.
- 결과 annotation 형식은 `  - 기대결과: ...`, `  - 결과: 성공|실패`, `  - 실패 사유: ...`로 고정한다.
- 성공 step에는 `실패 사유`를 쓰지 않고, 실패 step에는 반드시 `실패 사유`를 쓴다.
- E2E 실행 후에도 numbered step의 문장, 순서, 번호, 기능 설명은 절대 수정하지 않는다.
