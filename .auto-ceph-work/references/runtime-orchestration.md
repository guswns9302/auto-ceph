...
- 여기서 `리뷰 요청` 완료는 `07_SUMMARY.md` 갱신, canonical helper 기반 MR 생성 또는 재사용, Jira 작업 노트 요약 갱신, `08_LOOP.md` 동기화를 모두 포함한다.
...
- 티켓 loop가 terminal state에 도달하면 메인 세션은 `git status --short`로 변경 여부를 확인한다.
...
- `needs_retry` 자체로는 terminal git 후처리를 열지 않는다.
...
`리뷰 요청` 단계에서는 위 status 보장 외에도 `07_SUMMARY.md` 핵심 섹션과 `## Merge Request` 섹션을 `#### 리뷰 요청` 블록에 반영하고, `.auto-ceph-work/scripts/create_or_reuse_merge_request.js` helper로 `feature/<TICKET-ID>` -> `dev` MR을 생성 또는 재사용하고, `08_LOOP.md` 전문을 Jira description의 `### 루프 히스토리` 섹션에 동기화해야 한다.
