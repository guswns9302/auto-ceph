...
18. 메인 세션은 티켓 terminal 후 commit/push를 수행하지 않는다. 이 작업은 `리뷰 요청` 단계가 `.auto-ceph-work/scripts/commit_and_push_ticket_branch.sh`를 통해 소유한다.
19. `리뷰 요청` 단계의 기본 성공 커밋 메시지는 `feat(auto-ceph): finish <TICKET-ID>`다. `blocked`와 `failed` 종료는 각각 `chore(auto-ceph): block <TICKET-ID>`, `chore(auto-ceph): fail <TICKET-ID>`를 사용한다. `needs_retry`는 terminal 상태가 아니므로 종료 커밋 메시지로 사용하지 않는다.
20. `리뷰 요청` 단계의 non-MR git 작업은 `.auto-ceph-work/scripts/commit_and_push_ticket_branch.sh` canonical helper만 사용한다. helper는 현재 checkout 브랜치 upstream을 우선 사용하고, upstream이 없으면 티켓의 `remote`와 현재 티켓 브랜치 `feature/<TICKET-ID>`를 fallback push 대상으로 사용한다.
21. 티켓의 `remote`가 로컬 git remote에 없거나 `리뷰 요청` 단계의 commit/push helper가 실패하면 후처리 실패로 보고 즉시 종료한다.
22. 티켓 terminal 후에는 메인 세션이 `.auto-ceph-work/scripts/return_to_dev_branch.sh` canonical helper로 반드시 `dev`로 checkout 복귀해야 하며, 실패 시 전체 실행을 즉시 중단한다.
...
24. `리뷰 요청` 완료가 자동 루프의 기본 종료 지점이다. 여기서 완료는 `07_SUMMARY.md` 기본 요약 작성, ticket branch commit/push, canonical helper 기반 MR 생성 또는 재사용, helper 결과의 `07_SUMMARY.md` 반영, Jira description 최종 동기화를 모두 포함한다. 이 최종 동기화는 `#### 리뷰 요청` 요약과 top-level `### 루프 히스토리`의 `08_LOOP.md` 전문 반영을 함께 뜻한다.
...
27. `리뷰 요청` 단계는 후기 단계 중 유일하게 git mutation을 수행한다. `07_SUMMARY.md`의 non-MR 요약 섹션을 먼저 작성한 뒤 non-MR git은 `.auto-ceph-work/scripts/commit_and_push_ticket_branch.sh` canonical helper로만 처리하고, MR 관련 작업은 `.auto-ceph-work/scripts/create_or_reuse_merge_request.js` canonical helper로 처리해야 한다. MR helper는 `glab` CLI만 사용하며 `feature/<TICKET-ID>` -> `dev` 열린 MR이 있으면 재사용하고 없으면 생성해야 한다. helper 성공 후에는 `## Merge Request` 섹션을 `07_SUMMARY.md`에 반영해 handoff/Jira source of truth로 유지한다.
28. `리뷰 요청` 단계에서는 `07_SUMMARY.md`의 변경 사항/검증 결과/코드 리뷰 결과/`Merge Request` 핵심 메타를 `#### 리뷰 요청` 블록에 반영하고, `08_LOOP.md` 전문을 Jira description top-level `### 루프 히스토리` 섹션에 동기화해야 한다.
29. 이후 새 피드백은 다음 `$auto-ceph <TICKET-ID>` 실행에서 Jira 설명/작업 노트와 `루프 히스토리`를 우선 읽어 새 iteration으로 처리한다.
