...
27. `리뷰 요청` 단계에서는 `07_SUMMARY.md` 핵심 섹션을 `#### 리뷰 요청` 블록에 반영하고, `08_LOOP.md` 전문을 Jira description top-level `### 루프 히스토리` 섹션에 동기화해야 한다.
28. 이후 새 피드백은 다음 `$auto-ceph <TICKET-ID>` 실행에서 Jira 설명/작업 노트와 `루프 히스토리`를 우선 읽어 새 iteration으로 처리한다.
