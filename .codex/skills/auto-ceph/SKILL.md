...
8. Jira 상태 전이는 작업 노트와 별개로 다룬다. `문제 확인`은 `IN PROGRESS`, `수행` 완료는 `RESOLVE`, `검증` 시작은 `REVIEW`가 되어야 한다. `코드 리뷰`, `리뷰 요청`은 기본적으로 상태를 바꾸지 않는다.
...
21. `코드 리뷰`는 `검증`과 다르다. `검증`은 테스트/동작 확인이고, `코드 리뷰`는 현재 diff의 품질, 리스크, 테스트 충분성을 심사한다.
22. `코드 리뷰`에서 blocking finding이 있으면 retryable failure로 간주하고 같은 실행 안에서 즉시 `수행`으로 재진입해야 한다.
