...
24. `리뷰 요청` 완료가 자동 루프의 기본 종료 지점이다. 여기서 완료는 `07_SUMMARY.md` 갱신, canonical helper 기반 MR 생성 또는 재사용, Jira 작업 노트 요약 갱신, `08_LOOP.md` 동기화를 모두 포함한다.
...
27. `리뷰 요청` 단계에서는 `07_SUMMARY.md`에 `## Merge Request` 섹션을 포함해 MR 메타의 single source of truth로 유지해야 한다. MR 관련 작업은 모두 `.auto-ceph-work/scripts/create_or_reuse_merge_request.js` canonical helper로 처리하고, helper는 `glab` CLI만 사용하며 `feature/<TICKET-ID>` -> `dev` 열린 MR이 있으면 재사용하고 없으면 생성해야 한다.
...
- `리뷰 요청` stage 완료 조건은 `Jira 시작 기록 -> 07_SUMMARY.md 갱신 -> MR helper 성공 -> Jira 요약 기록 -> 08_LOOP.md 동기화` 순서를 모두 만족하는 것이다.
