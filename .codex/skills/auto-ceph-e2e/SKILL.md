...
7. helper `select` 결과인 compact selected cases에 포함된 모든 feature/step을 E2E 대상 범위로 확정하고, 선택된 `menu1`의 compact selected cases 전체를 feature/step 단위로 시나리오에 반영한다.
8. E2E 시나리오는 `e2e-scenario-template.md`의 `#### 테스트 시나리오`, `#### 기대 결과`, `#### 확인 범위` 구조만 사용한다.
9. 첫 단계는 항상 E2E config의 `url` 접속과 `id/pw` 로그인이며, 이후 단계는 compact selected cases의 `feature_name`, `menu_path`, `procedure`, `expected_result`를 feature/step 단위로 상세화해 조합한다.
10. E2E 시나리오에서 목록/검색/엑셀/생성/수정/삭제/상세 확인을 한 문장에 합치는 메뉴 단위 한 줄 요약은 금지한다. 테스트 케이스의 `선택 흐름`과 `예외 흐름`은 가능하면 별도 검증 단계로 분리한다.
11. E2E 실행 티켓은 Atlassian MCP `jira_create_issue`로 `project_key="CDS"`, `issue_type="Task"`, 제목 `[ACW E2E] <menu1> E2E 테스트`로 생성한다.
12. E2E 실행 티켓 description은 `e2e-jira-ticket-template.md` 구조를 따르고, 선택 메뉴, target case path, `### E2E 테스트 시나리오`, 빈 `### E2E 테스트 결과`, `### 작업 노트`를 포함한다.
...
16. E2E 실행 티켓 생성 시 작성한 `### E2E 테스트 시나리오`는 실행 기준선이며, 결과 기록, 종료 시간 기록, `DONE` 전이, 후속 티켓 생성 과정에서 기존 step 문장, 순서, 번호, 기능 설명을 변경하지 않는다.
17. E2E 종료 후 최신 description을 읽고 `step_results[]` 기준으로 `### E2E 테스트 시나리오`의 각 numbered step 아래에 annotation만 추가 또는 갱신한다.
18. 단계별 annotation 형식은 `  - 기대결과: ...`, `  - 결과: 성공|실패`, 실패 시 `  - 실패 사유: ...`로 고정한다. 성공 step에는 `실패 사유`를 쓰지 않는다.
19. `.auto-ceph-work/scripts/update_jira_work_note_section.js <description-file> "E2E 테스트 결과" section <result-block>` 방식으로 top-level `### E2E 테스트 결과` 섹션을 통계 요약 전용으로 생성 또는 교체한다. 최소 필드는 `전체 결과`, `성공`, `실패`, `실패 케이스`다.
20. 최신 description을 읽고 `.auto-ceph-work/scripts/update_jira_ticket_time_note.js <description-file> end`로 `티켓 종료 시간`을 기록한다.
21. 결과 기록과 종료 시간 기록이 끝나면 E2E 실행 티켓을 `DONE`으로 전이한다.
22. E2E 실패 기능/케이스마다 `[ACW] <E2E 티켓 ID> E2E 실패 후속 조치 - <기능명>` 제목의 후속 Jira `Task`를 사용자 입력 없이 생성한다.
23. 후속 티켓 생성 규칙, repo 판정, `remote: origin`, active sprint 배정, `TO DO` 전이는 `.auto-ceph-work/references/jira-create-template.md`를 따른다.
24. 모든 결과 기록, DONE 전이, 실패 후속 티켓 생성이 끝난 뒤 `기능`, `E2E 결과`, `실패 원인`, `후속 티켓` 컬럼의 Markdown 표를 출력한다.
25. `$auto-ceph-e2e` 실행 중에는 helper, config, skill, test-case 파일을 즉석 수정하지 않는다.
...
- 최초 작성된 `### E2E 테스트 시나리오`는 실행 기준선으로 보존하고, 실행 후에는 기존 step 아래 annotation만 추가한다.
