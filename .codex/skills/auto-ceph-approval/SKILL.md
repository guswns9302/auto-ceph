...
7. `.auto-ceph-work/references/e2e-execution-contract.md`
8. `.auto-ceph-work/references/jira-create-template.md`
9. `.auto-ceph-work/scripts/select_e2e_cases.js`
10. `.auto-ceph-work/scripts/approve_and_merge_review_mr.js`
11. `.auto-ceph-work/scripts/run_trombone_pipeline.sh`
12. `.codex/agents/aceph-approval-e2e.toml`

## Runtime Rules

1. 무인자 실행만 지원한다. 티켓 ID 인자는 받지 않는다.
2. 시작 즉시 Trombone config와 E2E config를 검증한다. `타겟 케이스` 파일은 repo root 기준 상대 경로로 해석하고, `.auto-ceph-work/scripts/select_e2e_cases.js`로 파싱 가능해야 한다.
...
12. 이후 단계는 티켓 내용과 `07_SUMMARY.md` 기준으로 관련 메뉴/기능을 판단해 `select_e2e_cases.js select <target-case-json> <menu1>`의 compact related cases만 사용한다. 판단 불가 시 전체 `v306.json`을 읽지 말고 `관련 케이스 없음`으로 최소 작성한다.
13. MR 작업은 `.auto-ceph-work/scripts/approve_and_merge_review_mr.js <TICKET-ID> feature/<TICKET-ID> dev`만 사용한다.
...
16. Trombone helper 성공은 `status=completed`와 `pipeline=<pipeline_prefix><repo>` 반환이다. Trombone 배포 실패 시 MR 성공 티켓 전체에 `Trombone 배포 실패 (<pipeline>)` 댓글을 남기고 E2E 및 `DONE` 전이를 수행하지 않는다.
17. Trombone helper/runtime failure는 실제 배포 실패로 라벨링하지 않고 별도 실패 원인으로 보고한다.
18. Trombone 완료 후 E2E 실행, 결과 검증, Jira 결과 기록, `DONE` 전이는 `.auto-ceph-work/references/e2e-execution-contract.md`를 따른다.
19. E2E 실패 티켓마다 `[ACW] <원본 티켓> E2E 실패 후속 조치` 제목의 후속 Jira `Task`를 사용자 입력 없이 생성한다. 후속 티켓 생성 규칙, repo 판정, active sprint 배정, `TO DO` 전이는 `.auto-ceph-work/references/jira-create-template.md`를 따른다.
20. 모든 E2E 실행, 원본 티켓 `DONE` 전이, 실패 후속 티켓 생성이 끝난 뒤 `티켓`, `E2E 결과`, `DONE 전이`, `실패 원인`, `후속 티켓` 컬럼의 Markdown 표를 출력한다.
21. 모든 E2E 및 DONE 전이가 끝난 후 처리된 모든 티켓에 `Trombone 파이프라인 실행 완료 (<pipeline>)` 댓글을 남긴다.
22. `$auto-ceph-approval` 실행 중에는 helper, config, skill, test-case 파일을 즉석 수정하지 않는다.
