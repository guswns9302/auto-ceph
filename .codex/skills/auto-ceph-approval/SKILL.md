...
- `$auto-ceph-approval`
  ...
  - Trombone 완료 후 대상 티켓별 E2E 테스트를 수행하고 성공 티켓을 `DONE`으로 전이한다.
...
36. Trombone helper는 `$CODEX_HOME/skills/playwright/scripts/playwright_cli.sh` wrapper를 기본 경로로 사용해야 하며, 필요하면 `PWCLI_BIN` override를 허용한다. 실행 전에 wrapper와 `run-code` 사용 가능 여부를 preflight로 확인해야 한다.
37. Trombone helper는 `login_url` 접속, `id/pw` 로그인, 좌측 메뉴 `빌드배포 > 파이프라인 관리` 진입, `${pipeline_prefix}${repo}` 검색, 배포환경 `dev` row의 `실행` 버튼 클릭, dev 파이프라인 완료 상태 polling을 순서대로 수행해야 한다.
38. 실행 버튼이 없거나 비활성화면 실패로 처리하고 즉시 종료한다. Playwright CLI 에러, session collision, malformed helper output은 실패이며 `status=completed`를 출력하면 안 된다.
39. Trombone helper 성공 조건은 실행 버튼 클릭 후 dev 파이프라인 완료 확인이다. 배포 완료 polling timeout, 실패 상태, 완료 상태 판별 실패는 모두 non-zero exit다.
40. Trombone helper가 `status=completed`와 `pipeline=<pipeline_prefix><repo>`를 반환한 뒤에만 E2E agent를 실행한다.
41. E2E 테스트는 `.codex/agents/aceph-approval-e2e.toml` custom agent를 티켓 단위로 순차 spawn해 수행한다.
42. E2E agent 입력에는 티켓 ID, Jira description의 `### E2E 테스트 시나리오`, `.auto-ceph-work/references/e2e-test-config.md`, `타겟 케이스` JSON, Playwright CLI wrapper 경로를 포함한다.
43. E2E agent는 `$CODEX_HOME/skills/playwright/scripts/playwright_cli.sh`를 사용하고, 결과를 `status=passed|failed`, `ticket_id`, `summary`, `evidence` 형태로 보고해야 한다.
44. E2E 실행 성공 후 Jira description에 top-level `### E2E 테스트 결과` 섹션을 생성 또는 교체한다. 위치는 `### E2E 테스트 시나리오` 다음, `### 작업 노트` 이전이다.
45. E2E 실행 성공 후 Atlassian MCP comment API로 해당 티켓에 `E2E 테스트 성공` 댓글을 남긴다.
46. E2E 실행 실패 시 Atlassian MCP comment API로 해당 티켓에 `E2E 테스트 실패` 댓글을 남기고, 해당 티켓을 `DONE`으로 전이하지 않은 채 전체 실행을 실패로 종료한다.
47. E2E 성공 댓글과 결과 기록이 끝난 뒤 Atlassian MCP로 해당 티켓 상태를 `DONE`으로 변경한다.
48. E2E 성공 후 Jira `DONE` 전이 실패는 해당 티켓 처리 실패로 보고 전체 실행을 실패로 종료한다.
49. 모든 E2E 성공 티켓의 `DONE` 전이까지 끝난 뒤, 이번 실행에서 MR approve/dev merge가 성공한 모든 티켓에 Atlassian MCP comment API로 `Trombone 파이프라인 실행 완료 (<pipeline_prefix><repo>)` 댓글을 추가한다.
50. Trombone 완료 댓글 중 하나라도 실패하면 전체 실행은 실패로 보고 최종 보고에 실패 티켓을 포함한다.
51. `$auto-ceph` stage 작업 노트는 계속 Jira description을 사용하고 comment API를 쓰지 않는다. `$auto-ceph-approval`의 comment API 사용은 approval status notification comment와 E2E result notification comment에만 허용되는 예외다.
52. `$auto-ceph-approval` 실행 중에는 helper, config, skill, test-case 파일을 즉석 수정하지 않는다. helper 호환성 문제는 현재 실행을 중단하고 Auto-Ceph repo에서 테스트 후 배포해야 한다.
...
- Trombone 완료 후 처리 티켓별 E2E agent를 순차 실행한다.
- E2E 성공 티켓은 Jira description에 `### E2E 테스트 결과`를 기록하고 `E2E 테스트 성공` 댓글을 남긴 뒤 `DONE`으로 전이한다.
- E2E 실패 티켓은 `E2E 테스트 실패` 댓글을 남기고 `DONE`으로 전이하지 않는다.
...
- 실행 중 helper, config, skill, test-case 파일을 수정하지 않으며, helper/preflight 실패는 즉시 중단 사유다.
- 종료 시에는 사용자 제외 티켓, REVIEW 전이 실패로 제외된 티켓, MR 처리 티켓, E2E 처리 티켓, DONE 전이 티켓, 중단된 티켓, Trombone 실행 여부를 짧게 보고한다.
