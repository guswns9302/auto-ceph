...
14. MR 작업은 `.auto-ceph-work/scripts/approve_and_merge_review_mr.js <TICKET-ID> feature/<TICKET-ID> dev`만 사용하고 세부 성공/실패 계약은 `.auto-ceph-work/references/mr-approval-contract.md`를 따른다.
15. MR helper 성공 직후 해당 티켓에 `MR approve / merge success` 댓글을 남긴다. 실패하면 즉시 전체 실행을 중단한다.
16. 모든 대상 티켓의 MR batch가 성공한 뒤 `.auto-ceph-work/scripts/run_trombone_pipeline.sh <REPO> <CONFIG-FILE>`를 한 번만 호출한다.
17. Trombone helper 성공/실패 stdout, 배포 실패 댓글, runtime failure 분류는 `.auto-ceph-work/references/trombone-deployment-contract.md`를 따른다.
