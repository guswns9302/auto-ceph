...
12. 이후 단계는 티켓 내용과 `07_SUMMARY.md` 기준으로 관련 메뉴/기능을 판단해 `.auto-ceph-work/scripts/select_e2e_cases.js select <target-case-json> <menu1>`의 compact related cases만 사용한다. 관련 cases가 있으면 `feature_name`, `menu_path`, `procedure`, `expected_result`를 feature/step 단위로 상세화하고, 판단 불가 시 전체 `v306.json`을 읽지 말고 `관련 케이스 없음`으로 최소 작성한다.
13. E2E 시나리오에서 목록/검색/엑셀/생성/수정/삭제/상세 확인을 한 문장에 합치는 메뉴 단위 한 줄 요약은 금지한다. 테스트 케이스의 `선택 흐름`과 `예외 흐름`은 가능하면 별도 검증 단계로 분리한다.
14. 각 티켓의 `### E2E 테스트 시나리오`는 MR approve 전에 작성 또는 교체한 뒤 실행 기준선으로 고정한다.
15. MR 작업은 `.auto-ceph-work/scripts/approve_and_merge_review_mr.js <TICKET-ID> feature/<TICKET-ID> dev`만 사용하고 세부 성공/실패 계약은 `.auto-ceph-work/references/mr-approval-contract.md`를 따른다.
16. MR helper 성공 직후 해당 티켓에 `MR approve / merge success` 댓글을 남긴다. 실패하면 즉시 전체 실행을 중단한다.
17. 모든 대상 티켓의 MR batch가 성공한 뒤 `.auto-ceph-work/scripts/run_trombone_pipeline.sh <REPO> <CONFIG-FILE>`를 한 번만 호출한다.
18. Trombone helper 성공/실패 stdout, 배포 실패 댓글, runtime failure 분류는 `.auto-ceph-work/references/trombone-deployment-contract.md`를 따른다.
19. Trombone 완료 후 E2E 실행, 결과 검증, Jira 결과 기록, `DONE` 전이는 `.auto-ceph-work/references/e2e-execution-contract.md`를 따른다.
20. E2E 결과 기록은 티켓별 최신 description에 대해 `step_results[]` 기준으로 `### E2E 테스트 시나리오`의 각 numbered step 아래에 annotation만 추가 또는 갱신한다. 기존 step 문장, 순서, 번호, 기능 설명은 수정하지 않는다.
21. 단계별 annotation 형식은 `  - 기대결과: ...`, `  - 결과: 성공|실패`, 실패 시 `  - 실패 사유: ...`로 고정한다. 성공 step에는 `실패 사유`를 쓰지 않는다.
22. `### E2E 테스트 결과` 섹션은 통계 요약 전용으로 갱신하며 최소 `전체 결과`, `성공`, `실패`, `실패 케이스`를 기록한다.
23. E2E 실패 티켓마다 `[ACW] <원본 티켓> E2E 실패 후속 조치` 제목의 후속 Jira `Task`를 사용자 입력 없이 생성한다. 후속 티켓 생성 규칙, repo 판정, active sprint 배정, `TO DO` 전이는 `.auto-ceph-work/references/jira-create-template.md`를 따른다.
24. 모든 E2E 실행, 원본 티켓 `DONE` 전이, 실패 후속 티켓 생성이 끝난 뒤 `티켓`, `E2E 결과`, `DONE 전이`, `실패 원인`, `후속 티켓` 컬럼의 Markdown 표를 출력한다.
25. 모든 E2E 및 DONE 전이가 끝난 후 처리된 모든 티켓에 `Trombone 파이프라인 실행 완료 (<pipeline>)` 댓글을 남긴다.
26. `$auto-ceph-approval` 실행 중에는 helper, config, skill, test-case 파일을 즉석 수정하지 않는다.
...
- MR approve 전에 작성한 `### E2E 테스트 시나리오`는 티켓별 실행 기준선으로 보존하고, E2E 후에는 기존 step 아래 annotation만 추가한다.
- `### E2E 테스트 결과`는 전체/성공/실패/실패 케이스 통계 요약만 기록한다.
